...
namespace Application.Helper
{
    public static class BusyTimeValidationHelper
    {
        ...
        public static (bool IsValid, string ErrorMessage) ValidateBusyTimePostRequest(BusyTimeDTORequest request)
        {
            if (request == null)
        ...
        public static (bool IsValid, string ErrorMessage) ValidateBusyTimePutRequest(BusyTimeDTORequest request)
        {
            if (request == null)
                return (false, "Request cannot be null.");
            ...
            foreach (var busyTime in request.BusyTimes)
            {
                if( busyTime.BusyTimeId == null)
                {
                    return (false, "BusyTimeId is required.");
                }
                // DayOfWeek
                if (string.IsNullOrWhiteSpace(busyTime.DayOfWeek))
                    return (false, "DayOfWeek is required.");
